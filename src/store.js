...
const state = {
    kefuName: "客服" + Math.floor(Math.random() * 100),
    // 输入的搜索值
    searchText: "",
    menus: menu,
    // 当前登录用户
    user: {
        name: "ratel",
        img: "/static/img/logo.ca2b07cc.png"
    },
    // 对话好友列表
    chatlist: [
        // {
...
const mutations = {
    ...
    sendMessage(state, msg1) {
        ...
        message.from = state.kefuName
        message.to = result.user.name
        message.time = new Date().toLocaleTimeString()
        message.msg = msg1.content
    ...
    send(state) {
        ...
        if (!msg) {
            ...
            router.push({ path: "/widgets2/kefu/chat" })
        }
    },
    addChat(state, chat) {
        state.chatlist = chat
    ...
    setPermission(state, arrays) {
        state.permissionData = arrays.concat([, 100, 200, 300])
    }
}
...
const getters = {
    ...
    getMenus(state) {
        const MenuFilter = state.menus.filter(item => {
            if (item.items) {
                item.items = item.items.filter(item3 => {
                    if (state.permissionData.includes(item3.permissionCode)) {
                        return item3
                    }
                })
            }

            if (state.permissionData.includes(item.permissionCode)) {
                return item
            }
        })
...
const actions = {
    ...
    addChatLis: ({ commit }, chat) => commit("addChat", chat),
    setMenus: ({ commit }, permission) => commit("setPermission", permission)
}
...
if (typeof WebSocket == "undefined") {
    ...
    let socketUrl = "ws://localhost:9090/estate/socket/" + state.kefuName
    ...
    socket.onmessage = function(msg) {
        ...
        if (data.userNames) {
            ...
            vuemain.$store.dispatch("addChatLis", chatList)
        } else if (data.msg) {
            // userNames不存在则是有人发消息
            let result = state.chatlist.find(session => session.user.name === data.from)
            setTimeout(() => {
